 ...
 */
@Component
public class CityNavigationServiceImpl implements ICityNavigationService {
	...
	public boolean isConnected(String start, String destination) throws Exception {
		...
		if (StringUtils.isEmpty(start) || StringUtils.isEmpty(destination)) {
			return false;
		}
		...
		if (start.equalsIgnoreCase(destination) && appLevelCityCache.containsKey(start)){
			logger.debug("if a Valid City is given as origin and destination then they are connected");
			return true;
		}
		if (!appLevelCityCache.containsKey(start) && !appLevelCityCache.containsKey(destination)) {
